// ExcelColumnRemover.js - Complete version with enhanced month separation feature
import React, { useState, useCallback } from 'react';
import { Calendar, Download } from 'lucide-react'; // Add these imports
import * as XLSX from 'xlsx'; // Make sure this is imported
...
// Add month separation functions with styling and filters
const getMonthNames = () => ({
  '01': 'January',
  '02': 'February', 
  '03': 'March',
...
const separateDataByMonths = (jsonData, dateColumnIndex, headerRowIndex, selectedHeaders, selectedMonths, monthCounts, allNewColumns, columnOrder, headers, addedCustomColumns) => {
  if (!jsonData || jsonData.length < 2 || dateColumnIndex === -1) {
    return null;
  }
  
  console.log('=== MONTH SEPARATION WITH FILTERS DEBUG ===');
  console.log('Total rows:', jsonData.length);
  console.log('Header row index:', headerRowIndex);
  console.log('Date column index:', dateColumnIndex);
  console.log('Selected headers to remove:', selectedHeaders);
  console.log('Selected months to filter:', selectedMonths);
  console.log('Column order:', columnOrder);
  console.log('Added columns:', addedCustomColumns);
  
  // STEP 1: First apply all the existing processing (filters, column additions, reordering)
  ...
  console.log('Processed data rows:', processedData.length - 1);
  console.log('Processed headers:', processedData[0]);
  
  // STEP 2: Find the date column in the processed data
  const processedHeaderRow = processedData[0];
  let processedDateColumnIndex = -1;
  
  // Try to find the date column by name in the processed headers
  const originalDateHeader = jsonData[headerRowIndex][dateColumnIndex];
  processedDateColumnIndex = processedHeaderRow.findIndex(header => header === originalDateHeader);
  
  if (processedDateColumnIndex === -1) {
    console.log('Date column not found in processed data - it may have been removed');
    return null;
  }
  
  console.log('Date column in processed data:', processedDateColumnIndex, processedHeaderRow[processedDateColumnIndex]);
  
  // STEP 3: Separate the processed data by months
  const monthData = {};
  const monthNames = getMonthNames();
  
  // Initialize month data structure
  Object.keys(monthNames).forEach(code => {
    monthData[code] = {
      name: monthNames[code],
      code: code,
      rows: []
    };
  });
  ...
  for (let i = 1; i < processedData.length; i++) {
    ...
    if (monthCode && monthData[monthCode]) {
      monthData[monthCode].rows.push(row);
      assignedRows++;
    } else {
      invalidDateRows++;
      console.log(`Row ${i}: Invalid date "${dateValue}" -> month code: ${monthCode}`);
    }
  }
  
  console.log(`Processed ${processedRows} rows, assigned ${assignedRows} to months, ${invalidDateRows} had invalid dates`);
  
  // Filter out months with no data and sort by month number
  const monthsWithData = Object.values(monthData)
    .filter(month => month.rows.length > 0)
    .sort((a, b) => a.code.localeCompare(b.code));
  
  console.log('Months with data:', monthsWithData.map(m => `${m.name}: ${m.rows.length} rows`));
  console.log('=== END MONTH SEPARATION WITH FILTERS DEBUG ===');
...
export default function ExcelColumnRemover() {
  ...
  // Add new state for month separation
  ...
  const onFileUpload = (e) => {
    ...
    setSeparatedData(null); // Reset separation data
  ...
  // UPDATED: Process month separation with all filters and styling
  const processSeparation = () => {
    if (!jsonData || dateColumnIndex === -1) {
      setError('Cannot separate by months: No data or date column found');
      return;
    }
    ...
    try {
      ...
      const allNewColumns = addedCustomColumns;
      
      const separated = separateDataByMonths(
        jsonData, 
        dateColumnIndex, 
        headerRowIndex,
        selectedHeaders,      // Pass current selections
        selectedMonths,       // Pass current month filters
        monthCounts,         // Pass month counts
        allNewColumns,       // Pass new columns
        columnOrder,         // Pass column order
        headers,             // Pass original headers
        addedCustomColumns   // Pass custom columns
      );
            ...
            <div className="bg-green-50 rounded-lg p-4 border border-green-200 mb-4">
             ...
             <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 mb-4">
               {separatedData.monthsWithData.map((month) => (
                 <div key={month.code} className="bg-white rounded-lg border border-gray-200 p-3 shadow-sm">
